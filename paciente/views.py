...
def home(request):
  if request.user.is_authenticated:
    ...
    especilidade = Especialidades.objects.all()
    return render(request, 'home.html', {'medicos': medicos, 'especialidades': especilidade, 'is_medico': is_medico(request.user)})
...
def escolher_horario(request, id_dados_medicos):
  if request.method == 'GET':
    ...
    return render(request, 'agendar_horario.html', {'medico': medico, 'datas_abertas': datas_abertas, 'is_medico': is_medico(request.user)})
